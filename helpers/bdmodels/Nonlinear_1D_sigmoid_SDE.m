...
%
function sys = Nonlinear_1D_sigmoid_SDE(Kij)    
    % number of nodes
    n = size(Kij, 1);
    ...
    sys.pardef = [
        ...
        struct('name','A', 'value',4.5*ones(n,1), 'lim',[0, 10])
        struct('name','B', 'value',0.00, 'lim',[0, 5])
        struct('name','Iamp', 'value',1, 'lim',[0, 10])
        struct('name','tau', 'value',15, 'lim',[0, 10])
        struct('name','T', 'value',30, 'lim',[0, 10])
    ...
    sys.panels.bdLatexPanel.latex = {
        ...
        '';
        '$d x_i = f \left[ -A_i \, x_i + \left( \sum_{j = 1}^{n} K_{ij} x_j \right) + s_i \, I(t) \right] \, dt + \sigma_i \, d N_i (t), \quad \forall \, i = 1, \cdots, n$';
        '';
    };
...
function Stimulus(ax,t,sol,Kij,A,B,Iamp,tau,T,s,sigma)
    % ax: current axis
    % t: current time step
...
% deterministic part
function [F, Iapp] = sdeF(t,Y,Kij,A,B,Iamp,tau,T,s,sigma)
...
% stochastic part
function [G] = sdeG(t,Y,Kij,A,B,Iamp,tau,T,s,sigma)
