...
function sys = R_Wong_Wang_ODE(Kij)    
    % number of nodes
    n = size(Kij, 1);
...
function Stimulus(ax,t,sol,Kij,a,b,d,gamma,tau_s,w,J_N,Iamp,tau,T,s)
    % ax: current axis
    % t: current time step
    ...
    for idx = 1:numel(sol.x)
        [~,iapp] = odefun( ...
            sol.x(idx), ...
            sol.y(:,idx), ...
            Kij,a,b,d,gamma,tau_s,w,J_N,Iamp,tau,T,s ...
            );
        Iapp(:, idx) = iapp;
    end
...
% deterministic part
function [F, Iapp] = odefun(t,Y,Kij,a,b,d,gamma,tau_s,w,J_N,Iamp,tau,T,s)
