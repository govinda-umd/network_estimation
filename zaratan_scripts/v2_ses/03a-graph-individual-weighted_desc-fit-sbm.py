...
def transformation(args, y):
    if args.transform == 'arctanh':
        y.a = 2 * np.arctanh(y.a)
    return y

def create_state(args, g):

    y = g.ep.weight.copy()
    y = transformation(args, y)
...
def setup_args():
    ...
    args.transform = sys.argv[8] # arctanh
...
def main():
    ...
    SBM_path = (
        ...
        f'/{sbm_name(args)}/transform-{args.transform}/B-{args.B}'
    )
    os.system(f'mkdir -p {SBM_path}')
    
    state, state_args = create_state(args, g)
    args.niter = 10
