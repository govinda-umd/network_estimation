...
def setup_args():
    ...
    # animal
    args.sub = int(sys.argv[13]) # 1, 2, ... 9
...
def indiv_level_modes(args, sub):
    gfile = sorted(glob.glob(f'{args.GRAPH_path}/*', recursive=True))[0]
    g = gt.load_graph(gfile)

    sbm_files = sorted(glob.glob(f'{args.SBM_path}/task-rest/{args.SBM}/*/desc-partition-modes.pkl', recursive=True))
...
def main():
    args = setup_args()
    # ---------
    args.BASE_path = f'{os.environ["HOME"]}/scratch/new_mouse_dataset/roi-results-v3'
    ...
    with open(f'{folder}/{args.SBM}_samples-{args.total_samples}_desc-df.pkl', 'wb') as f:
