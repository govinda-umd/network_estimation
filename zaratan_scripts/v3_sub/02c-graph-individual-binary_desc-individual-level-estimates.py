...
def indiv_level_modes(args, sub):
    # graph for sub-{sub}
    gfile = sorted(glob.glob(f'{args.GRAPH_path}/sub-{sub}*', recursive=True))[0]
    g = gt.load_graph(gfile)

    # collect modes per chain, for all chains of the animal
    sbm_files = sorted(glob.glob(f'{args.SBM_path}/sub-{sub}*/{args.SBM}/*/desc-partition-modes.pkl', recursive=True))
