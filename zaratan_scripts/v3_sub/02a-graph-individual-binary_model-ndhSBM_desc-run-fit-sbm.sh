...
for B in "${Bs[@]}"
do
    sbatch 02a-graph-individual-binary_model-ndhSBM_desc-fit-sbm.sh \
        ${SOURCE} ${SPACE} ${BRAIN_DIV} ${NROIS} ${RES} \
        ${GRAPH_DEF} ${GRAPH_METHOD} ${THRESHOLDING} ${EDGE_DENSITY} ${DATA_UNIT} \
        ${WAIT} ${MCMC_LEN} ${B} ${GAMMA} ${SEED}
done

# bash 02a-graph-individual-binary_model-ndhSBM_desc-run-fit-sbm.sh \
# allen ccfv2 whl 446 200 pearson signed 20 sub