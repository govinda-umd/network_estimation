...
def setup_args():
    ...
    args.UNIT = sys.argv[2] # seswise
    args.RECONST_method = sys.argv[3] # 'normal_dist'
    args.graph_file = int(sys.argv[4]) # file of graph, line number (index) in all_graphs.txt
    args.sbm = sys.argv[5] # p d o h
    args.dc = sys.argv[6] == '1' # degree corrected?
    args.wait = int(sys.argv[7]) # 1000
    args.force_niter = int(sys.argv[8]) # 100,000
    args.SEED = int(sys.argv[9]) # random seed
...
def main():
    ...
    BASE_path = f'{os.environ["HOME"]}/scratch/mouse_dataset'
    all_graphs_file = (
        f'{BASE_path}/roi_results'
        f'/{args.DESC}/{args.UNIT}/reconstructed_graph/{args.RECONST_method}'
        f'/all_graphs.txt'
    )
    with open(f'{all_graphs_file}', 'r') as f:
    ...
    graph_file = '_'.join(fs[-1].split('_')[:-1] + [f'desc-graph.gt.gz'])
    graph_file = '/'.join(fs[:-2] + [f'graphs', f'{graph_file}'])
    print(args.graph_file)
    
    if not fs[-6] == args.DESC: return None
...
# sbatch 02c-desc-fit-sbm.sh spatial 225 True whl 162 seswise 15 a 1 12000 100000 100
