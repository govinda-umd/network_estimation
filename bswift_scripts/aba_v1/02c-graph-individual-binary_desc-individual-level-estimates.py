...
def setup_args():
    ...
    args.force_niter = int(sys.argv[13]) #100,000
    args.total_samples = int(sys.argv[14]) #10,000 from a chain for aggregating
    args.gamma = float(sys.argv[15]) #2.0
    args.SEED = int(sys.argv[16]) #100
...
def main():
    ...
    args.SBM_path = (
        ...
        f'/{args.SBM}/B-*/cond-{args.cond}'
    ) 
    args.ESTIM_path = (
        f'{args.ROI_RESULTS_path}/estimates'
        f'/{args.SBM}/cond-{args.cond}'
    )
