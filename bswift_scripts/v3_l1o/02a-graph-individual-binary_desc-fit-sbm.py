...
def setup_args():
    ...
    args.wait = int(sys.argv[5]) # 120
    args.total_samples = int(sys.argv[6]) # 80,000
    args.B = int(sys.argv[7]) # 1
    args.gamma = float(sys.argv[8]) # 2.0, used only in mod.max.state
    args.SEED = int(sys.argv[9]) # random seed

    # we will run mcmc_eq `num_iters` times
    # args.force_niter = 10
