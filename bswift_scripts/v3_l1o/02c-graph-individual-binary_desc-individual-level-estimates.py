...
def setup_args():
    ...
    args.DATA_UNIT = sys.argv[8] #f'l1o'
    ...
    args.all_graphs_file = sys.argv[13]
    args.graph_file = int(sys.argv[14]) # file of graph, line number (index) in all_graphs.txt
...
def collect_sbm_files(args, sbm_files, g):
    ...
    for file in tqdm(sbm_files):
        ...
        match = re.search(r'sub-([^_]+)', file)
        sub = match.group(1)
        
        match = re.search(r'sbm-([^/]+)', file)
        sbm = match.group(1)
        
        match = re.search(r'/B-([^/]+)', file)
        B = match.group(1)
...
def indiv_level_modes(args, g, sub):
...
def main():
    ...
    match = re.search(
        r'(sub-[a-zA-Z0-9]+_task-[a-zA-Z0-9]+)', 
        args.graph_file,
    )
    sub = match.group(1)
    ...
    # ---------
    modes_df = indiv_level_modes(args, g, sub)

    folder = f'{args.ESTIM_path}/individual/{sub}/partition-modes'
